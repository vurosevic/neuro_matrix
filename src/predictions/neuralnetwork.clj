...
(defn hidden-error
  ...
  (let [delta-scalar (entry o-deltas current)
    ...
    (mv! 1.0 (trans weights) (prepare-unit-vector (inc current)) 0.0 result)
    ))
...
(defn backpropagation
  ...
  (let [hidden-layer (:hidden-layer network)
    ...
    (do
      ...
      ;; network output is in temp-matrix-3
...
(defn predict
  ...
  (let [net-input-dim (ncols (:hidden-layer network))
    ...
    (if (= net-input-dim input-vec-dim)
      ...
      (throw (Exception. (str "Error. Input dimensions is not correct. Expected dimension is: " net-input-dim)))
      )))
